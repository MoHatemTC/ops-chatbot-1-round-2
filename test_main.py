...
def test_chat_stream_unauthorized():
    payload = {"message": "Hello", "thread_id": "session-123", "cohort_id": "alexandria-university-cohort-1"}
...
def test_chat_stream_forbidden_cohort():
    ...
    payload = {"message": "Hello", "thread_id": "session-123", "cohort_id": "wrong-cohort-id"}
...
def test_chat_stream_success():
    ...
    headers = {"Authorization": f"Bearer {token}"}
    payload = {"message": "Hello Bot", "thread_id": "session-123", "cohort_id": "alexandria-university-cohort-1"}
    response = client.post("/chat/stream", json=payload, headers=headers)
    assert response.status_code == 200
