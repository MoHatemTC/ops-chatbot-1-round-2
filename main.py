...
def chatbot_node(state: dict) -> dict:
    ...
    return {"messages": ["Hello Bot [Stub Response]: This is a simulated multi-turn response for testing. Genuine RAG knowledge retrieval and citations will be integrated in Sprint 2."]}
...
async def chat_stream(payload: ChatRequest, current_user: dict = Depends(get_current_user)):
    ...
    if user_cohort != payload.cohort_id:
        raise HTTPException(
            ...
            detail="Unauthorized access to this cohort data"
        )
